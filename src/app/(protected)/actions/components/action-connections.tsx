...
export function ActionConnections({ id }: {id: string}) {
  ...
  const integrations = (action?.appliedToIntegrations || []).map(({integration}) => (
    integration
  )).filter((_, index) => index < 4);

  const hasMore = (action?.appliedToIntegrations?.length || 0) > integrations.length;
  const lengthDiff = (action?.appliedToIntegrations?.length || 0) - integrations.length;
