import { ActionRunResponse, DataInput, Integration, useAction, useIntegrationApp } from "@integration-app/react"
...
export function ExecuteActionModal({
  ...
  const [selectIntegrationMissing, setSelectIntegrationMissing] = useState<boolean>(false);
      ...
      <DialogContent className="sm:max-w-3xl">
        ...
        <div className="flex gap-6 flex-col sm:flex-row">
          ...
          <div className="flex-1 flex flex-col gap-2">
            ...
            {!executionError && <ScrollArea className="max-h-60 h-full flex-1 min-h-20 border rounded-md p-2">
        ...
        <DialogFooter className="flex-row justify-end">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant={selectIntegrationMissing ? "destructive" : "outline"}>
                {selectedIntegration ? (
                  <>
            ...
            <DropdownMenuContent className="w-56">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuRadioGroup value={selectedIntegration?.key} onValueChange={(key) => {
                setSelectedIntegration(
                  action?.appliedToIntegrations?.find(({ integration }) => integration.key === key)?.integration
                );
                setSelectIntegrationMissing(false)
              }}>
                {action?.appliedToIntegrations?.map(({integration}) => (
                  <DropdownMenuRadioItem key={integration.id} value={integration.key}>
                    {integration.name}
                  </DropdownMenuRadioItem>
                ))}
